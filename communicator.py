...
class Communicator:
    ...
    def publish(self, message):
        ...
        return json.loads(urllib.request.urlopen(self.makePublish(message)).read().decode("utf-8"))

    def subscribe(self, timetoken = None):
        if timetoken is None:
            timetoken = self.timetoken
        result = json.loads(urllib.request.urlopen(self.makeSubscribe(timetoken)).read().decode("utf-8"))
